	    ...
	    <tr align="center">
	        ...
	        <td width="80%" align="${status.first?'center':'left'}">
	           ...
	           <c:choose>
		        ...
		        <c:otherwise>    
		                ${article.user.id==rootUserAtAdminLook.id? "<font size='6' color='red'>楼主</font></c:if>" : article.user.name} 回复：
		              <!-- 找到父贴名称 -->
		                <c:forEach var="articleTemp" items="${requestScope.articles}" varStatus="status">
		                    <c:if test="${article.pid==articleTemp.id}">
		                       ${articleTemp.user.id==rootUserAtAdminLook.id? "<font size='6' color='red'>楼主</font></c:if>" : articleTemp.user.name} 
		                    </c:if>
		                </c:forEach>
		        </c:otherwise>
             </c:choose>
...
<!-- -----------------------------------展开讨论----------------------------------------------------------------------- -->
	    ...
	    <table border="0" width="100%" bgcolor="#E0EEE0">
			  <tr><td  rowspan=2 width="10%"><img src="headerImage/user${article.user.id}.jpg" alt="${user.name}" width="40" height="40"></img><br/>${article.user.id==rootUserAtAdminLook.id? "<font size='6' color='red'>楼主</font></c:if>" : article.user.name}</td>
			      <td>回复：
		                <c:forEach var="articleTemp" items="${requestScope.articles}" varStatus="status">
		                    <c:if test="${article.pid==articleTemp.id}">
		                       ${articleTemp.user.id==rootUserAtAdminLook.id? "<font size='6' color='red'>楼主</font></c:if>" : articleTemp.user.name} :${article.content}
		                    </c:if>
		                </c:forEach>
		          </td>
		      </tr>
...
<!-- -----------------------------------讨论结束----------------------------------------------------------------------- -->
 ...
 <c:if test="${article.id==rootId||articles[statusFirst.index+1].pid==rootId||statusFirst.last}">
   ...
   <hr/>
	 ...
	 <center>
	   <a href="AdminArticleAction?action=pageNoSelected&tag=${requestScope.tag}&keyWord=${requestScope.keyWord}&pageNo=${requestScope.indexPageNo}&pageCount=${requestScope.indexPageCount}"  onclick=" return linkClick(this)">返回到主页</a>
	 </center>
  </body>
</html>
